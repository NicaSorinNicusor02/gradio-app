...
# ---- Fault cards (gallery) ----
def build_fault_cards(run: str, max_cards: int = 200):
    if not run: return []
    p = os.path.join(run, "detections.json")
    if not os.path.exists(p): return []
    dets = json.load(open(p, "r"))
    if not dets: return []
    files = sorted_images(os.path.join(run, "images"))
    out_dir = os.path.join(run, "cards")
    os.makedirs(out_dir, exist_ok=True)
    cards = []
    produced = 0
    for j, d in enumerate(dets):
        if produced >= int(max_cards):
            break
        ...
        try:
            ...
            cv.rectangle(vis, (x1, y1), (x2, y2), (0, 255, 0), 3, cv.LINE_AA)
            label = d.get("label", "fault")
            score = float(d.get("score", 1.0))
            cv.putText(vis, f"{label} {score:.2f}", (x1, max(20, y1 - 8)),
                       cv.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2, cv.LINE_AA)
        except Exception:
            pass
        out_path = os.path.join(out_dir, f"card_{j:05d}.jpg")
        try:
            cv.imwrite(out_path, vis)
        except Exception:
            continue
        caption = f"{os.path.basename(fp)} · {d.get('label','fault')} {float(d.get('score',1.0)):.2f} · {_severity(float(d.get('score',1.0)))}"
        cards.append([out_path, caption])
        produced += 1
    return cards

def fault_detail_from_card(evt_or_value, run: str):
    """Given a clicked gallery card, return detailed preview image and markdown.
    Accepts either a gr.SelectData event, a filepath string, or an integer index.
    Expects cards generated by build_fault_cards (card_XXXXX.jpg naming).
    """
    if not run:
        return None, "", "", "", "", "Upload images first."
    # Resolve selection into a detection index or file path
    det_idx = None
    sel_path = None
    # Case: event with .index (preferred for Gallery)
    try:
        if hasattr(evt_or_value, "index") and isinstance(evt_or_value.index, int):
            det_idx = int(evt_or_value.index)
    except Exception:
        pass
    # Case: raw string filepath
    if det_idx is None and isinstance(evt_or_value, str):
        if os.path.isfile(evt_or_value):
            sel_path = evt_or_value
    # Case: integer index -> map to cards/card_{idx}.jpg
    elif det_idx is None and isinstance(evt_or_value, int):
        pth = os.path.join(run, "cards", f"card_{evt_or_value:05d}.jpg")
        if os.path.isfile(pth):
            sel_path = pth
    else:
        # Case: event with .value
        try:
            if hasattr(evt_or_value, "value"):
                v = evt_or_value.value
            ...
            pass
    # If we don't have an explicit index, try to parse from filename
    if det_idx is None and sel_path:
        try:
            base = os.path.basename(sel_path)
            name, _ = os.path.splitext(base)
            det_idx = int(name.split("_")[-1])
        except Exception:
            det_idx = None
    p = os.path.join(run, "detections.json")
    if det_idx is None or not os.path.exists(p):
        return None, "", "", "", "", ""
    dets = json.load(open(p, "r"))
    ...
    d = dets[det_idx]
    files = sorted_images(os.path.join(run, "images"))
    img_i = int(d.get("image_index", -1))
    if img_i < 0 or img_i >= len(files):
        return None, "", "", "", "", ""
    # Prefer the clicked card image (already has overlay); fall back to original
    src_path = sel_path if (sel_path and os.path.isfile(sel_path)) else files[img_i]
    im = cv.imread(src_path, cv.IMREAD_COLOR)
    if im is None:
        return None, "", "", "", "", ""
    # If we loaded the original image (not the card), overlay the box and label
    if src_path == files[img_i]:
        try:
            x1, y1, x2, y2 = [int(v) for v in d["bbox"]]
            cv.rectangle(im, (x1, y1), (x2, y2), (0, 200, 255), 4, cv.LINE_AA)
            label = d.get("label", "fault"); score = float(d.get("score", 1.0))
            cv.putText(im, f"{label} {score:.2f}", (x1, max(24, y1 - 10)),
                       cv.FONT_HERSHEY_SIMPLEX, 0.8, (0, 200, 255), 2, cv.LINE_AA)
        except Exception:
            pass
    severity = _severity(float(d.get('score',1.0)))
    img_rgb = cv.cvtColor(im, cv.COLOR_BGR2RGB)
    fname = os.path.basename(files[img_i])
    label = d.get('label','fault')
    score_str = f"{float(d.get('score',1.0)):.2f}"
    analysis = "Analysis placeholder"
    return img_rgb, fname, label, score_str, severity, analysis
